...
export function async_test(func, name, properties) {
  ...
  if(func) {
    ...
    if(value !== undefined) {
      let msg =
        'Test named "' + test_name + '" passed a function to `async_test` that returned a value (' + value + ').';
...
function is_node(object) {
  ...
  try {
    var has_node_properties =
      'nodeType' in object && 'nodeName' in object && 'nodeValue' in object && 'childNodes' in object;
  } catch(e) {
      ...
      if(is_node(val)) {
        switch (val.nodeType) {
          ...
          case Node.DOCUMENT_NODE:
            return (
              'Document node with ' + val.childNodes.length + (val.childNodes.length == 1 ? ' child' : ' children')
            );
...
function cleanup_done(test, errors, bad_value_count) {
  if(errors.length || bad_value_count) {
    ...
    tests.status.message =
      "Test named '" + test.name + "' specified " + total + " 'cleanup' function" + (total > 1 ? 's' : '');
...
Tests.prototype.setup = function(func, properties) {
  ...
  if(func) {
    try {
      ...
      this.status.status =
        e instanceof OptionalFeatureUnsupportedError ? this.status.PRECONDITION_FAILED : this.status.ERROR;
      ...
      .map(assert => {
        ...
        if(output_len[output_len.length - 1] > 50) {
          output_args = output_args.map(
            (x, i) => (i > 0 ? '  '.repeat(prefix_len) : '') + x + (i < output_args.length - 1 ? ',\n' : ''),
          );
        } else {
...
function make_message(function_name, description, error, substitutions) {
  ...
  const node_form = substitute(
    ['{text}', '${function_name}: ${description}' + error],
    merge({ function_name: function_name, description: description ? description + ' ' : '' }, substitutions),
  );
