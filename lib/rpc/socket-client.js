...
/* No dependency on 'lws.so'/'../lws/protocols.js' at the top level for the
   browser-loadable createWebSocketClient() below - createWsClientHandlers()
 ...
 * and reassembled instead of each piece independently failing to parse.
 *
...
export function createWsClientHandlers(rpcOptions = null) {
  ...
  const handlers = createSocketClientHandlers(codec, (wsi, request) => wsi.write(codec.encode(request)), false);
  return { ...handlers, message: reassembleFragments(handlers.message) };
}
