...
export class TCPSocketStream {
  ...
  static #ref = new ContextRefCounter(() => {
    TCPSocketStream.#ctx?.destroy();
    TCPSocketStream.#ctx = undefined;
    TCPSocketStream.#adapter = undefined;
  });
  ...
  constructor(options = {}, serverOptions) {
    ...
    this.#wsi = TCPSocketStream.#ctx.clientConnect({
      ...
      protocol: 'raw',
      ...rest,
      sslConnection: (rest.sslConnection ?? 0) | (tls ? LCCSCF_USE_SSL | tlsConnectFlags(tls) : 0),
    });

    this.#opened = session.opened;
    this.#closed = session.closed;

    TCPSocketStream.#ref.track(this, this.#opened, this.#closed);
  }
