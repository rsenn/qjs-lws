...
import { noop, assert, assert_default } from './simple-queue.js';
...
var rethrowAssertionErrorRejection = DEBUG
  ? e => {
...
var promiseResolve = originalPromiseResolve;
...
function defaultReaderClosedPromiseResetToRejected(reader, reason) {
  assert_default(reader._closedPromise_resolve === void 0);
  assert_default(reader._closedPromise_reject === void 0);
...
var number_isfinite_default = NumberIsFinite;
...
var math_trunc_default = MathTrunc;
...
function convertUnsignedLongLongWithEnforceRange(value, context) {
  ...
  if(!number_isfinite_default(x) || x === 0) {
    return 0;
  }
  return x;
}
...
var _a, _b, _c;
var SymbolAsyncIterator = (_c = (_b = Symbol.asyncIterator) != null ? _b : (_a = Symbol.for) == null ? void 0 : _a.call(Symbol, 'Symbol.asyncIterator')) != null ? _c : '@@asyncIterator';
...
export class ReadableStreamAsyncIteratorImpl {
  ...
  _nextSteps() {
    ...
    assert_default(reader._ownerReadableStream !== void 0);
    let resolvePromise;
    let rejectPromise;
    const promise = newPromise((resolve, reject) => {
      resolvePromise = resolve;
...
var number_isnan_default = NumberIsNaN;
...
var number_isinteger_default = NumberIsInteger;
...
function SetUpReadableByteStreamController(stream, controller, startAlgorithm, pullAlgorithm, cancelAlgorithm, highWaterMark, autoAllocateChunkSize) {
  assert_default(stream._readableStreamController === void 0);
  if(autoAllocateChunkSize !== void 0) {
...
function convertByobReadOptions(options, context) {
  var _a2;
  assertDictionary(options, context);
  const min = (_a2 = options == null ? void 0 : options.min) != null ? _a2 : 1;
  return {
...
function ExtractHighWaterMark(strategy, defaultHWM) {
  ...
  if(number_isnan_default(highWaterMark) || highWaterMark < 0) {
...
function convertQueuingStrategy(init, context) {
  ...
  const highWaterMark = init == null ? void 0 : init.highWaterMark;
  const size = init == null ? void 0 : init.size;
  return {
...
function convertUnderlyingSink(original, context) {
  ...
  const abort = original == null ? void 0 : original.abort;
  const close = original == null ? void 0 : original.close;
  const start = original == null ? void 0 : original.start;
  const type = original == null ? void 0 : original.type;
  const write = original == null ? void 0 : original.write;
  return {
...
function WritableStreamAbort(stream, reason) {
  ...
  (_a2 = stream._writableStreamController._abortController) == null ? void 0 : _a2.abort(reason);
...
function WritableStreamStartErroring(stream, reason) {
  assert_default(stream._storedError === void 0);
  assert_default(stream._state === 'writable');
  const controller = stream._writableStreamController;
  assert_default(controller !== void 0);
...
function WritableStreamFinishInFlightWrite(stream) {
  assert_default(stream._inFlightWriteRequest !== void 0);
...
function WritableStreamFinishInFlightClose(stream) {
  assert_default(stream._inFlightCloseRequest !== void 0);
  ...
  assert_default(stream._pendingAbortRequest === void 0);
  assert_default(stream._storedError === void 0);
}
...
function WritableStreamMarkCloseRequestInFlight(stream) {
  assert_default(stream._inFlightCloseRequest === void 0);
  assert_default(stream._closeRequest !== void 0);
...
export class WritableStreamDefaultWriter {
  ...
  releaseLock() {
    ...
    assert_default(stream._writer !== void 0);
...
function SetUpWritableStreamDefaultController(stream, controller, startAlgorithm, writeAlgorithm, closeAlgorithm, abortAlgorithm, highWaterMark, sizeAlgorithm) {
  assert_default(IsWritableStream(stream));
  assert_default(stream._writableStreamController === void 0);
...
function WritableStreamDefaultControllerGetChunkSize(controller, chunk) {
  if(controller._strategySizeAlgorithm === void 0) {
...
function defaultWriterClosedPromiseResetToRejected(writer, reason) {
  assert_default(writer._closedPromise_resolve === void 0);
  assert_default(writer._closedPromise_reject === void 0);
  assert_default(writer._closedPromiseState !== 'pending');
...
function defaultWriterReadyPromiseReset(writer) {
  assert_default(writer._readyPromise_resolve === void 0);
  assert_default(writer._readyPromise_reject === void 0);
  defaultWriterReadyPromiseInitialize(writer);
}
...
function getFromGlobal() {
  var _a2;
  const ctor = (_a2 = globals) == null ? void 0 : _a2.DOMException;
...
function ReadableStreamPipeTo(source, dest, preventClose, preventAbort, preventCancel, signal) {
  ...
  assert_default(signal === void 0 || isAbortSignal(signal));
...
export class ReadableStreamDefaultController {
  ...
  close() {
    if(!IsReadableStreamDefaultController(this)) {
      throw defaultControllerBrandCheckException2('close');
    }
...
function convertUnderlyingDefaultOrByteSource(source, context) {
  ...
  const autoAllocateChunkSize = original == null ? void 0 : original.autoAllocateChunkSize;
  const cancel = original == null ? void 0 : original.cancel;
  const pull = original == null ? void 0 : original.pull;
  const start = original == null ? void 0 : original.start;
  const type = original == null ? void 0 : original.type;
  return {
...
function convertIteratorOptions(options, context) {
  assertDictionary(options, context);
  const preventCancel = options == null ? void 0 : options.preventCancel;
...
function convertPipeOptions(options, context) {
  assertDictionary(options, context);
  const preventAbort = options == null ? void 0 : options.preventAbort;
  const preventCancel = options == null ? void 0 : options.preventCancel;
  const preventClose = options == null ? void 0 : options.preventClose;
...
function convertReadableWritablePair(pair, context) {
  ...
  const readable = pair == null ? void 0 : pair.readable;
  assertRequiredField(readable, 'readable', 'ReadableWritablePair');
  assertReadableStream(readable, `${context} has member 'readable' that`);
  const writable = pair == null ? void 0 : pair.writable;
...
export class ReadableStream {
  constructor(rawUnderlyingSource = {}, rawStrategy = {}) {
    ...
    if(underlyingSource.type === 'bytes') {
      ...
      assert_default(underlyingSource.type === void 0);
      const sizeAlgorithm = ExtractSizeAlgorithm(strategy);
      const highWaterMark = ExtractHighWaterMark(strategy, 1);
  ...
  get locked() {
    if(!IsReadableStream(this)) {
      throw streamBrandCheckException2('locked');
    }
  ...
  cancel(reason = void 0) {
    if(!IsReadableStream(this)) {
      return promiseRejectedWith(streamBrandCheckException2('cancel'));
    }
  ...
  getReader(rawOptions = void 0) {
    if(!IsReadableStream(this)) {
      throw streamBrandCheckException2('getReader');
    }
  ...
  pipeThrough(rawTransform, rawOptions = {}) {
    if(!IsReadableStream(this)) {
      throw streamBrandCheckException2('pipeThrough');
    }
  ...
  pipeTo(destination, rawOptions = {}) {
    if(!IsReadableStream(this)) {
      return promiseRejectedWith(streamBrandCheckException2('pipeTo'));
    }
  ...
  tee() {
    if(!IsReadableStream(this)) {
      throw streamBrandCheckException2('tee');
    }
  ...
  values(rawOptions = void 0) {
    if(!IsReadableStream(this)) {
      throw streamBrandCheckException2('values');
    }
...
function convertTransformer(original, context) {
  assertDictionary(original, context);
  const cancel = original == null ? void 0 : original.cancel;
  const flush = original == null ? void 0 : original.flush;
  const readableType = original == null ? void 0 : original.readableType;
  const start = original == null ? void 0 : original.start;
  const transform = original == null ? void 0 : original.transform;
  const writableType = original == null ? void 0 : original.writableType;
  return {
...
export class TransformStream {
  ...
  get readable() {
    if(!IsTransformStream(this)) {
      throw streamBrandCheckException3('readable');
    }
  ...
  get writable() {
    if(!IsTransformStream(this)) {
      throw streamBrandCheckException3('writable');
    }
...
export class TransformStreamDefaultController {
  ...
  get desiredSize() {
    if(!IsTransformStreamDefaultController(this)) {
      throw defaultControllerBrandCheckException3('desiredSize');
    }
  ...
  enqueue(chunk = void 0) {
    if(!IsTransformStreamDefaultController(this)) {
      throw defaultControllerBrandCheckException3('enqueue');
    }
  ...
  error(reason = void 0) {
    if(!IsTransformStreamDefaultController(this)) {
      throw defaultControllerBrandCheckException3('error');
    }
  ...
  terminate() {
    if(!IsTransformStreamDefaultController(this)) {
      throw defaultControllerBrandCheckException3('terminate');
    }
...
function SetUpTransformStreamDefaultController(stream, controller, transformAlgorithm, flushAlgorithm, cancelAlgorithm) {
  ...
  assert_default(stream._transformStreamController === void 0);
...
function TransformStreamDefaultSinkWriteAlgorithm(stream, chunk) {
  ...
  if(stream._backpressure) {
    ...
    assert_default(backpressureChangePromise !== void 0);
...
function TransformStreamDefaultSourcePullAlgorithm(stream) {
  ...
  assert_default(stream._backpressureChangePromise !== void 0);
  TransformStreamSetBackpressure(stream, false);
