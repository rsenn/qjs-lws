...
const _queueMicrotask = callback => {
...
const SymbolAsyncIterator =
  (_c =
    (_b = Symbol.asyncIterator) != null
      ? _b
      : (_a = Symbol.for) == null
        ? undefined
        : _a.call(Symbol, 'Symbol.asyncIterator')) != null
    ? _c
    : '@@asyncIterator';
...
function ReadableByteStreamControllerFillHeadPullIntoDescriptor(controller, size, pullIntoDescriptor) {
  assert_default(
    controller._pendingPullIntos.length === 0 || controller._pendingPullIntos.peek() === pullIntoDescriptor,
  );
...
function WritableStreamDefaultControllerGetChunkSize(controller, chunk) {
  if(controller._strategySizeAlgorithm === undefined) {
    assert_default(
      controller._controlledWritableStream._state == 'erroring' ||
...
function defaultWriterReadyPromiseInitialize(writer) {
  assign(writer, {
    _readyPromise: newPromise((resolve, reject) =>
      assign(writer, { _readyPromise_resolve: resolve, _readyPromise_reject: reject }),
    ),
...
function InitializeTransformStream(
  ...
  assign(stream, {
    _readable: CreateReadableStream(
      startAlgorithm,
      pullAlgorithm,
      cancelAlgorithm,
      readableHighWaterMark,
...
function assign(obj, ...args) {
  for(let props of args)
    for(let prop in props)
      Object.defineProperty(obj, prop, { value: props[prop], configurable: true, writable: true });
}
