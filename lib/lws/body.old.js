...
import { isPrototypeOf, assign, readWholeStream } from './util.js';
...
export class Body {
  ...
  blob() {
    ...
    if(this.#chunks || this.#queue || this.#bodyStream) return readWholeStream(this.body).then(arr => Promise.resolve(new Blob(arr)));
  }
  ...
  arrayBuffer() {
    ...
    if(this.#chunks || this.#queue || this.#bodyStream) return readWholeStream(this.body).then(arr => Promise.resolve(new Blob(arr).arrayBuffer()));
  }
  ...
  text() {
    ...
    if(this.#chunks || this.#queue || this.#bodyStream) return readWholeStream(this.body).then(arr => Promise.resolve(new Blob(arr).text()));
  }
