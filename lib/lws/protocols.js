  ...
  onHttp = wsi => {
    ...
    if(hasBody(req.headers)) {
      /* Content-Type is already known (headers arrive before body) - a
         multipart request gets its own MultipartParser, fed alongside the
         usual Body sink below, so req.formData() (body.js) can drain it
         incrementally instead of buffering the whole body via .text(). */
      if(isMultipart(req.headers)) req._multipart = new MultipartParser(wsi);
