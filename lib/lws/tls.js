 ...
 * derive `sslConnection`/context `options` bits from the same
 ...
 */
import { generateSelfSignedCert as nativeGenerateSelfSignedCert, toArrayBuffer, toString, LWS_SERVER_OPTION_ALLOW_NON_SSL_ON_SSL_PORT, LWS_SERVER_OPTION_CREATE_VHOST_SSL_CTX, LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT, LWS_SERVER_OPTION_PEER_CERT_NOT_REQUIRED, LWS_SERVER_OPTION_REDIRECT_HTTP_TO_HTTPS, LCCSCF_ALLOW_EXPIRED, LCCSCF_ALLOW_INSECURE, LCCSCF_ALLOW_SELFSIGNED, LCCSCF_SKIP_SERVER_CERT_HOSTNAME_CHECK, } from 'lws.so';
 ...
 *
 * Built on lws_x509_create_cert() (libwebsockets/lws-x509.h), so this works
 * under any TLS backend lws is built with - no direct OpenSSL dependency.
 * That native API only carries a single subject/SAN name though, so unlike
 * the old OpenSSL-based implementation, only *one* of commonName/altNames[0]
 * ends up as both the cert's CN and its sole SAN entry - multiple SANs
 * aren't supported any more.
 *
 * @param  {object}   [options]
 * @param  {string}   [options.commonName='localhost']
 * @param  {string[]} [options.altNames]    Used as the CN/SAN (altNames[0])
 *                                          only when commonName is absent -
 *                                          see above, no longer multi-entry.
...
export function tlsContextOptions(tls) {
  ...
  /* LWS_SERVER_OPTION_IGNORE_MISSING_CERT is deliberately NOT set here:
     resolveTls() above already guarantees opts.cert/opts.key are present
     (self-signed fallback) by this point, so it's never needed - and lws's
     own lws_tls_generic_cert_checks() (tls-network.c) only ever looks at
     the *filepath* fields to decide "is a cert present", never the mem
     ones. With that flag set, a mem-only cert (no ssl_cert_filepath) reads
     as "missing" and openssl-server.c's lws_tls_server_vhost_backend_init()
     frees vhost->tls.ssl_ctx back to NULL - which lws_context_init_alpn()
     then dereferences unconditionally, segfaulting in
     SSL_CTX_set_alpn_select_cb() - confirmed via gdb backtrace. */
  const options =
    LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT |
    LWS_SERVER_OPTION_CREATE_VHOST_SSL_CTX |
    ('rejectUnauthorized' in opts && !opts.rejectUnauthorized ? LWS_SERVER_OPTION_PEER_CERT_NOT_REQUIRED | LWS_SERVER_OPTION_ALLOW_NON_SSL_ON_SSL_PORT | LWS_SERVER_OPTION_REDIRECT_HTTP_TO_HTTPS : 0);
  ...
  return {
    ...
    ssl_private_key_password: opts.privateKeyPassword,
    ssl_cipher_list: opts.cipherList,
    tls1_3_plus_cipher_list: opts.tls13PlusCipherList,
    client_ssl_ca: opts.ca,
    client_ssl_cert: opts.cert,
    client_ssl_private_key: opts.key,
    client_ssl_private_key_password: opts.privateKeyPassword,
    client_ssl_cipher_list: opts.cipherList,
    client_tls_1_3_plus_cipher_list: opts.tls13PlusCipherList,
  };
}

/**
 * The crypto-relevant `sslConnection` (LCCSCF_*) flags for one outbound
 ...
 *
 * @param  {true|object} tls
 * @return {number}      `sslConnection` (LCCSCF_*) bits to OR in, or `0`.
 */
