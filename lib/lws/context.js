import { CONTEXT_PORT_NO_LISTEN, getLogLevelColour, getLogLevelName, LLL_ERR, LLL_USER, LLL_INFO, LLL_WARN, logLevel, LWS_SERVER_OPTION_ADOPT_APPLY_LISTEN_ACCEPT_CONFIG, LWSContext } from 'lws.so';
import { tlsContextOptions } from './tls.js';
import { typeIsObject } from './util.js';
import { assert_default as assert } from './assert.js';
import { loadFile, open, puts, getenviron } from 'std';
import * as os from 'os';
import { stat, realpath } from 'os';

// lws-context.c's service tick looks up os.setTimeout on globalThis, which qjsm never defines
globalThis.os ??= os;

let { DEBUG, HOSTNAME, TRAFFIC } = getenviron();

if(DEBUG == '0') DEBUG = 0;

/* RX/TX byte counts (socket_flush()/lwsjs_socket_respond()/
   lwsjs_callback_protocol(), lws-socket.c/lws-protocol.c) only ever reach
   this callback at all under LLL_USER - OR it into the mask here so
   TRAFFIC works standalone, without also needing DEBUG. */
const trafficLog = TRAFFIC ? open(TRAFFIC, 'a+') : null;

logLevel((DEBUG | TRAFFIC ? LLL_USER : 0) | LLL_ERR /*| LLL_WARN*/, (level, msg) => {
  const isTraffic = /\s(RX|TX)\s/.test(msg);

  if(!isTraffic) puts('\x1b[2K\r\x1b[0;30m' + getLogLevelColour(level).replace(/\b38\b/g, '48') + centerPad(8, getLogLevelName(level)) + '\x1b[0m ' + msg + '\n');
  else if(trafficLog) {
    trafficLog.puts(msg + '\n');
    trafficLog.flush();
  }
...
const resolvConfFile = 'resolv.conf';
