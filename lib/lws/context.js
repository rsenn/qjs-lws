...
export function createContext(info = {}) {
  ...
  if(typeIsObject(info.protocols) && !Array.isArray(info.protocols)) {
    const protocols = [];

    for(let name in info.protocols) {
      assert(typeIsObject(info.protocols[name]));

      info.protocols[name].name = name;
      protocols.push(info.protocols[name]);
    }
