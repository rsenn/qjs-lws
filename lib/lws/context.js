...
const { DEBUG, HOSTNAME, TRAFFIC } = getenviron();
...
logLevel((DEBUG || TRAFFIC ? LLL_USER | (DEBUG > 1 ? LLL_INFO : 0) : 0) | (TRAFFIC ? LLL_USER : 0) | LLL_ERR | LLL_WARN, (level, msg) => {
  if(DEBUG && level == LLL_USER) puts('\x1b[2K\r\x1b[0;30m' + getLogLevelColour(level).replace(/\b38\b/g, '48') + centerPad(8, getLogLevelName(level)) + '\x1b[0m ' + msg + '\n');

  if(trafficLog && /\s(RX|TX)\s\d+\sbytes\b/.test(msg)) {
    trafficLog.puts(msg + '\n');
    trafficLog.flush();
  }
});
