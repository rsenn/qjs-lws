export class Headers {
  ...
  constructor(headers) {
    if(headers instanceof Headers) {
      ...
      // No init provided, valid per spec
    } else if(headers === null || typeof headers !== 'object') {
      // Per WHATWG spec, null and non-object types are invalid
      throw new TypeError('Headers constructor: invalid init type');
    } else {
      // Valid object init
   ...
   *
   * WHATWG spec requires lexicographic sorting (case-insensitive, by normalized name).
   */
  keys() {
    const names = Object.keys(this.#map).sort();
    const items = [];

    for(const name of names) items.push(name);
  ...
  values() {
    ...
    for(const name of names) {
      const v = this.#map[name];
      if(Array.isArray(v)) for(const item of v) items.push(item);
      else items.push(v);
    }
  ...
  entries() {
    ...
    for(const name of names) {
      const v = this.#map[name];
      if(Array.isArray(v)) for(const item of v) items.push([name, item]);
      else items.push([name, v]);
    }
