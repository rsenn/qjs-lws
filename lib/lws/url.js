 ...
 *  - IPv4/IPv6 literal parsing and serialization delegate to native
 *    LWSSockAddr46 (lws_sa46_parse_numeric_address() /
 *    lws_sa46_write_numeric_address()) rather than the spec's own IPv4/IPv6
 *    parser algorithms. This is stricter than the spec in one way - legacy
 *    hex/octal IPv4 forms (e.g. "0x7f000001", "0177.0.0.1") are rejected as
 *    invalid hosts instead of being normalized to dotted-decimal - since
 *    endsInANumber() still uses the spec's own per-label check to decide
 *    whether a domain label "ends in a number" (so such a label is still
 *    routed into IPv4 parsing and fails there, rather than being kept as an
 *    opaque domain label).
 */
import { toString, toArrayBuffer, LWSSockAddr46 } from 'lws.so';
...
function parseIPv4(input) {
  const addr = new LWSSockAddr46(input).address;

  if(!addr || addr.byteLength !== 4) return null;

  const b = new Uint8Array(addr);
...
function serializeIPv4(address) {
  ...
  return new LWSSockAddr46(bytes).host;
}

function parseIPv6(input) {
  const addr = new LWSSockAddr46(input).address;

  if(!addr || addr.byteLength !== 16) return null;

  const b = new Uint8Array(addr);
  const pieces = [];

  for(let i = 0; i < 16; i += 2) pieces.push((b[i] << 8) | b[i + 1]);
...
function serializeIPv6(address) {
  ...
  for(let i = 0; i < 8; i++) {
    bytes[i * 2] = address[i] >> 8;
    bytes[i * 2 + 1] = address[i] & 0xff;
  }

  return new LWSSockAddr46(bytes).host;
}

const FORBIDDEN_HOST_CODEPOINTS = new Set([0x00, 0x09, 0x0a, 0x0d, 0x20, 0x23, 0x2f, 0x3a, 0x3c, 0x3e, 0x3f, 0x40, 0x5b, 0x5c, 0x5d, 0x5e, 0x7c].map(c => String.fromCharCode(c)));
...
export class URLSearchParams {
  ...
  constructor(init) {
    if(init === undefined) {
      ...
      define(this, { [kList]: parseFormURLEncoded(init[0] === '?' ? init.slice(1) : init) });
    } else if(init instanceof URLSearchParams) {
      define(this, { [kList]: init[kList].map(pair => [...pair]) });
      ...
      define(this, { [kList]: parseFormURLEncoded(String(init)) });
    }
  }
  ...
  delete(name, value) {
    ...
    if(value !== undefined) {
      value = String(value);
      define(this, { [kList]: this[kList].filter(([n, v]) => !(n === name && v === value)) });
    } else {
      define(this, { [kList]: this[kList].filter(([n]) => n !== name) });
    }
...
Object.defineProperties(URLSearchParams.prototype, {
  [kURL]: { configurable: true, writable: true, enumerable: false },
  [kList]: { configurable: true, writable: true, enumerable: false },
});
...
export class URL {
  constructor(url, base) {
    ...
    define(this, { [kRecord]: parsedURL });

    const query = new URLSearchParams();

    define(query, { [kURL]: this });

    if(parsedURL.query !== null) query[kList] = parseFormURLEncoded(parsedURL.query);

    define(this, { [kURL]: query });
  }
  ...
  set href(value) {
    ...
    define(this, { [kRecord]: parsed });
    define(this[kURL], { [kList]: parsed.query !== null ? parseFormURLEncoded(parsed.query) : [] });
  }
  ...
  set search(value) {
    ...
    if(s === '') {
      url.query = null;
      define(this[kURL], { [kList]: [] });
      return;
    }

    url.query = '';
    basicURLParse(s[0] === '?' ? s.slice(1) : s, null, url, 'query');
    define(this[kURL], { [kList]: parseFormURLEncoded(url.query) });
  }
...
Object.defineProperties(URL.prototype, {
  [kRecord]: { configurable: true, writable: true, enumerable: false },
  [kURL]: { configurable: true, writable: true, enumerable: false },
});
