...
/**
 * @param  {Array<ArrayBuffer>} chunks  an Array of chunks.
 * @return {ArrayBuffer}
 */
export function concatArrayBuffer(chunks) {
  const totalSize = chunks.reduce((n, ch) => n + ch.byteLength, 0);
  const arr = new Uint8Array(totalSize);
  let pos = 0;

  for(let chunk of chunks) {
    /* Uint8Array.set() needs a typed array/array-like of byte values, not a
       bare ArrayBuffer - passing one silently copies nothing since an
       ArrayBuffer has no indexed properties of its own. */
    arr.set(ArrayBuffer.isView(chunk) ? chunk : new Uint8Array(chunk), pos);
...
/**
 * A `ReadableStream` paired with its own writable-side sink -
 * `{ write(buf), close(), error(err) }` - for feeding it from code that
 * isn't the stream's own `start()`/`pull()` (e.g. native callbacks).
 *
 * @return {[ReadableStream, {write, close, error}]}
 */
export function readableStreamSink() {
  let controller;
  return [
    new ReadableStream({
    ...
    {
      write: buf => controller.enqueue(new Uint8Array(buf)),
      close: () => controller.close(),
