...
export class AbortSignal extends EventTarget {
  ...
  static any(iterable) {
    const controller = new AbortController();
    /* Materialize once: `iterable` may be a one-shot iterator, and clean()
       needs to walk the same signals a second time to remove listeners. */
    const signals = [...iterable];
    const handlers = new Map();

    function clean() {
      for(const signal of signals) signal.removeEventListener('abort', handlers.get(signal));
    }
    ...
    for(const signal of signals) {
      ...
      /* EventTarget.dispatchEvent() invokes listeners as plain calls (no
         `this` binding to the firing target), so `signal` must be captured
         via closure rather than read off `this`. */
      const handler = () => {
        controller.abort(signal.reason);
        clean();
      };
