...
export class ReadableByteStreamController {
  ...
  close() {
    ...
    if(P._closeRequested) throw new TypeError('The stream has already been closed; do not close it again!');
    const state = P.controlledReadableByteStream._state;
  ...
  enqueue(chunk) {
    ...
    if(P._closeRequested) throw new TypeError('stream is closed or draining');
    const state = P.controlledReadableByteStream._state;
  ...
  [PullSteps](readRequest) {
    ...
    const autoAllocateChunkSize = P._autoAllocateChunkSize;
...
function ReadableByteStreamControllerCallPullIfNeeded(controller) {
  ...
  if(P._closeRequested) return false;
  ...
  assert(!P._pullAgain);
  P._pulling = true;
  const pullPromise = P._pullAlgorithm();
  uponPromise(
    pullPromise,
    () => {
      P._pulling = false;
...
function ReadableByteStreamControllerFillHeadPullIntoDescriptor(controller, size, pullIntoDescriptor) {
  ...
  assert(P._byobRequest === null);
  pullIntoDescriptor.bytesFilled += size;
}

function ReadableByteStreamControllerHandleQueueDrain(controller) {
  const P = CTRL(controller);
  assert(P.controlledReadableByteStream._state == 'readable');
  if(P.queueTotalSize === 0 && P._closeRequested) {
...
function ReadableByteStreamControllerInvalidateBYOBRequest(controller) {
  const P = CTRL(controller);
  if(P._byobRequest === null) return;
  P._byobRequest._associatedReadableByteStreamController = undefined;
...
function ReadableByteStreamControllerClearAlgorithms(controller) {
  Object.assign(CTRL(controller), { _pullAlgorithm: undefined, cancelAlgorithm: undefined });
}

function ReadableByteStreamControllerClose(controller) {
  const P = CTRL(controller);
  const stream = P.controlledReadableByteStream;
  if(P._closeRequested || stream._state != 'readable') return;
  if(P.queueTotalSize > 0) {
    P._closeRequested = true;
    return;
  }
...
function ReadableByteStreamControllerEnqueue(controller, chunk) {
  ...
  if(ReadableStreamHasDefaultReader(stream)) {
    const reader = P.controlledReadableByteStream._reader;
...
function ReadableByteStreamControllerGetBYOBRequest(controller) {
  const P = CTRL(controller);
  if(P._byobRequest === null && P.pendingPullIntos.length > 0) {
    const firstDescriptor = P.pendingPullIntos.peek();
...
function SetUpReadableByteStreamController(stream, controller, startAlgorithm, pullAlgorithm, cancelAlgorithm, highWaterMark, autoAllocateChunkSize) {
  ...
  P._pullAlgorithm = pullAlgorithm;
  P.cancelAlgorithm = cancelAlgorithm;
  P._autoAllocateChunkSize = autoAllocateChunkSize;
  P.pendingPullIntos = new SimpleQueue();
...
function ReadableStreamBYOBReaderRead(reader, view, min, readIntoRequest) {
  ...
  if(stream._state == 'errored') readIntoRequest._errorSteps(stream._storedError);
    ...
    if(stream._readableStreamController.pendingPullIntos.length > 0) {
      stream._readableStreamController.pendingPullIntos.push(pullIntoDescriptor);
    ...
    if(stream.CTRL(_readableStreamController).queueTotalSize > 0) {
      ...
      if(stream.CTRL(_readableStreamController)._closeRequested) {
        const e = new TypeError('Insufficient bytes to fill elements in the given buffer');
...
function WritableStreamAbort(stream, reason) {
  ...
  stream._writableStreamController.abortReason = reason;
...
function WritableStreamDefaultWriterWrite(writer, chunk) {
  ...
  if(P._strategySizeAlgorithm === undefined) {
    assert(P.controlledWritableStream._state == 'erroring' || P.controlledWritableStream._state == 'errored');
    return 1;
  }
  try {
    chunkSize = P._strategySizeAlgorithm(chunk);
  } catch(chunkSizeE) {
...
export class WritableStreamDefaultController {
  ...
  get signal() {
    ...
    const P = CTRL(this);
    if(P._abortController === undefined) throw new TypeError('WritableStreamDefaultController.prototype.signal is not supported');
    return P._abortController.signal;
  }
  ...
  error(e = undefined) {
    ...
    const state = CTRL(this).controlledWritableStream._state;
...
function SetUpWritableStreamDefaultController(stream, controller, startAlgorithm, writeAlgorithm, closeAlgorithm, abortAlgorithm, highWaterMark, sizeAlgorithm) {
  ...
  P._abortController = new AbortController();
  P.started = false;
  P._strategySizeAlgorithm = sizeAlgorithm;
  P.strategyHWM = highWaterMark;
  P._writeAlgorithm = writeAlgorithm;
  P._closeAlgorithm = closeAlgorithm;
...
function WritableStreamDefaultControllerClearAlgorithms(controller) {
  Object.assign(CTRL(controller), { _writeAlgorithm: undefined, _closeAlgorithm: undefined, abortAlgorithm: undefined, _strategySizeAlgorithm: undefined });
}
...
function WritableStreamDefaultControllerAdvanceQueueIfNeeded(controller) {
  ...
  if(value === closeSentinel) {
    ...
    assert(P.queue.length === 0);
    const sinkClosePromise = P._closeAlgorithm();
    ...
    const sinkWritePromise = P._writeAlgorithm(value);
    uponPromise(
      sinkWritePromise,
      () => {
        assert(stream._inFlightWriteRequest !== undefined);
...
function WritableStreamDefaultControllerErrorIfNeeded(controller, error) {
  if(CTRL(controller).controlledWritableStream._state == 'writable') WritableStreamDefaultControllerError(controller, error);
}
...
export class ReadableStreamDefaultController {
  ...
  [PullSteps](readRequest) {
    ...
    if(P.queue.length > 0) {
      ...
      if(P._closeRequested && P.queue.length === 0) {
        ReadableStreamDefaultControllerClearAlgorithms(this);
        ReadableStreamClose(stream);
      } else {
...
function ReadableStreamDefaultControllerClearAlgorithms(controller) {
  Object.assign(CTRL(controller), { _pullAlgorithm: undefined, cancelAlgorithm: undefined, _strategySizeAlgorithm: undefined });
}
...
function ReadableStreamDefaultControllerGetDesiredSize(controller) {
  const P = CTRL(controller);
  const state = P.controlledReadableStream._state;
  if(state == 'errored') return null;
  if(state == 'closed') return 0;
  return P.strategyHWM - P.queueTotalSize;
}
...
function ReadableStreamDefaultControllerCanCloseOrEnqueue(controller) {
  const P = CTRL(controller);
  const state = P.controlledReadableStream._state;
  if(!P._closeRequested && state == 'readable') return true;
  return false;
}
  ...
  function pullWithDefaultReader() {
    ...
    const readRequest = {
      ...
      _closeSteps: () => {
        ...
        if(branch1._readableStreamController.pendingPullIntos.length > 0) ReadableByteStreamControllerRespond(branch1._readableStreamController, 0);
        if(branch2._readableStreamController.pendingPullIntos.length > 0) ReadableByteStreamControllerRespond(branch2._readableStreamController, 0);
        if(!canceled1 || !canceled2) resolveCancelPromise(undefined);
      },
      _errorSteps: () => (reading = false),
    };
    ...
    const readIntoRequest = {
      ...
      _closeSteps: chunk => {
        ...
        if(chunk !== undefined) {
          ...
          if(!byobCanceled) ReadableByteStreamControllerRespondWithNewView(byobBranch._readableStreamController, chunk);
          if(!otherCanceled && otherBranch._readableStreamController.pendingPullIntos.length > 0) ReadableByteStreamControllerRespond(otherBranch._readableStreamController, 0);
        }
  ...
  constructor(rawTransformer = {}, rawWritableStrategy = {}, rawReadableStrategy = {}) {
...

    {
      ...
      function abortAlgorithm(reason) {
        const controller = stream._transformStreamController;
        const P = CTRL(controller);
        if(P._finishPromise !== undefined) return P._finishPromise;
        const readable = stream._readable;
        P._finishPromise = newPromise((resolve, reject) => {
          P._finishPromise_resolve = resolve;
          P._finishPromise_reject = reject;
        });
      ...
      function closeAlgorithm() {
        ...
        const flushPromise = P._flushAlgorithm();
        TransformStreamDefaultControllerClearAlgorithms(controller);
        uponPromise(
          flushPromise,
          () => {
      ...
      assign(this, { _transformStreamController: undefined });
    }
...
export class TransformStreamDefaultController {
  ...
  get desiredSize() {
    ...
    const readableController = CTRL(this).controlledTransformStream._readable._readableStreamController;
...
function TransformStreamDefaultControllerClearAlgorithms(controller) {
  Object.assign(CTRL(controller), { _transformAlgorithm: undefined, _flushAlgorithm: undefined, cancelAlgorithm: undefined });
}
...
function SetUpTransformStreamDefaultController(stream, controller, transformAlgorithm, flushAlgorithm, cancelAlgorithm) {
  ...
  P._transformAlgorithm = transformAlgorithm;
  P._flushAlgorithm = flushAlgorithm;
  P.cancelAlgorithm = cancelAlgorithm;
}
...
function TransformStreamDefaultControllerPerformTransform(controller, chunk) {
  ...
  const transformPromise = P._transformAlgorithm(chunk);
...
function defaultControllerFinishPromiseResolve(controller) {
  const P = CTRL(controller);
  if(P._finishPromise_resolve === undefined) return;
  P._finishPromise_resolve();
  P._finishPromise_resolve = undefined;
  P._finishPromise_reject = undefined;
}

function defaultControllerFinishPromiseReject(controller, reason) {
  const P = CTRL(controller);
  if(P._finishPromise_reject === undefined) return;
  setPromiseIsHandledToTrue(P._finishPromise);
  P._finishPromise_reject(reason);
  P._finishPromise_resolve = undefined;
  P._finishPromise_reject = undefined;
}
