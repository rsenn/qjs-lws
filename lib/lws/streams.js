...
import { isPrototypeOf, typeIsObject, define, assign, setFunctionName } from './util.js';
...
function ReadableByteStreamControllerRespondInternal(controller, bytesWritten) {
  ...
  if(state == 'closed') {
    ...
    assert_default(firstDescriptor.bytesFilled + bytesWritten <= firstDescriptor.byteLength);
...
function SetUpReadableByteStreamController(stream, controller, startAlgorithm, pullAlgorithm, cancelAlgorithm, highWaterMark, autoAllocateChunkSize) {
  ...
  if(autoAllocateChunkSize !== undefined) {
    assert_default(NumberIsInteger(autoAllocateChunkSize));
...
export class ReadableStream {
  constructor(rawUnderlyingSource = {}, rawStrategy = {}) {
    ...
    if(underlyingSource.type == 'bytes') {
      ...
      else pullAlgorithm = () => promiseResolvedWith(undefined);
      if(underlyingSource.cancel !== undefined) cancelAlgorithm = reason => underlyingSource.cancel(reason);
      else cancelAlgorithm = () => promiseResolvedWith(undefined);
...
export class TransformStream {
  constructor(rawTransformer = {}, rawWritableStrategy = {}, rawReadableStrategy = {}) {
    ...
    flushAlgorithm = transformer.flush !== undefined ? () => transformer.flush(controller) : () => promiseResolvedWith(undefined);
    cancelAlgorithm = transformer.cancel !== undefined ? reason => transformer.cancel(reason) : () => promiseResolvedWith(undefined);
