...
function ReadableByteStreamControllerCommitPullIntoDescriptor(stream, pullIntoDescriptor) {
  ...
  if(pullIntoDescriptor.readerType == 'default') {
    ...
    const reader = STRM(stream).reader;
    const R = READ(reader);
    assert(R(reader).readIntoRequests.length > 0);
...
function ReadableStreamAddReadIntoRequest(stream, readIntoRequest) {
  ...
  STRM(stream).READ(reader).readIntoRequests.push(readIntoRequest);
}

function ReadableStreamGetNumReadIntoRequests(stream) {
  return STRM(stream).READ(reader).readIntoRequests.length;
}
...
function WritableStreamAbort(stream, reason) {
  ...
  STRM(stream).writableStreamController.abortReason = reason;
  (_a2 = STRM(stream).writableStreamController.abortController) == null ? undefined : _a2.abort(reason);
...
export class WritableStreamDefaultController {
  ...
  error(e = undefined) {
    ...
    const state = CTRL(this).STRM(controlledWritableStream).state;
...
function WritableStreamDefaultControllerErrorIfNeeded(controller, error) {
  if(CTRL(controller).STRM(controlledWritableStream).state == 'writable') WritableStreamDefaultControllerError(controller, error);
}
