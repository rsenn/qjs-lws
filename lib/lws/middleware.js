...
export function json(opts = {}) {
  ...
  return async function jsonBody(req, res, next) {
    ...
    try {
      ...
      if(!text) { req.body = null; return next(); }
      ...
      if(/limit/.test(e.message)) { res.status(413).type('text/plain').end('Payload Too Large'); return; }
...
export function raw(opts = {}) {
  ...
  return async function rawBody(req, res, next) {
    const ct = req.headers['content-type'] || '';
    if(!re.test(ct)) return next();
    try { req.body = await req.readBody(limit); }
    catch(e) { res.status(413).type('text/plain').end('Payload Too Large'); return; }
    return next();
  };
}
...
export function text(opts = {}) {
  ...
  return async function textBody(req, res, next) {
    const ct = req.headers['content-type'] || '';
    if(!re.test(ct)) return next();
    try { req.body = await req.readText(limit); }
    catch(e) { res.status(413).type('text/plain').end('Payload Too Large'); return; }
    return next();
  };
}
...
export function cookies() {
  return function cookieParser(req, res, next) { void req.cookies; return next(); };
}
...
function arr(v) { return Array.isArray(v) ? v : String(v).split(/\s*,\s*/); }
...
export function secure(opts = {}) {
  ...
  return function secureMiddleware(req, res, next) {
    for(const [k, v] of Object.entries(headers))
      if(v !== false && v != null && !res.headers.has(k)) res.set(k, v);
    return next();
  };
}
