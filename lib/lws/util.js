...
export function verbose(name, ...args) {
  console.log('\x1b[2K\r\x1b[0;31mUSER\x1b[0m ' + padEnd(name + '', 32), console.config({ compact: true, depth: Infinity, maxBufferLength: 8, maxArrayLength: 16 }), LC, ...args);
}

export function debug(...args) {
  let level = 1;

  if(typeof args[0] == 'number') level = args.shift();

  let debugLevel = +process.env.DEBUG;
  if(isNaN(debugLevel)) debugLevel = 1;

  if(level <= debugLevel) verbose(...args);
}
