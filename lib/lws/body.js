...
import { define, typeIsObject, isPrototypeOf, isAsyncIterable, isIterable, isView } from './util.js';
...
export class Body {
  ...
  async blob() {
    ...
    // No body (constructor never got one) - a real WHATWG Request/Response
    // resolves to an empty Blob here rather than throwing.
    if(body == null) return new Blob([]);

    return new Blob(await readWholeStream(body));
  }
  ...
  async arrayBuffer() {
    ...
    // No body (constructor never got one) - a real WHATWG Request/Response
    // resolves to an empty buffer here rather than throwing.
   ...
   */
  async text() {
    return toString(await this.arrayBuffer(), true);
  }

  /**
   * Return the body as a Uint8Array
   *
   * @return {Promise<Uint8Array>}  Promise that resolves to a Uint8Array
   */
  async bytes() {
...
define(Body.prototype, { body: null, _bodyInit: null });
