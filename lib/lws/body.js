import { readableStreamCallback, readableStreamController, readWholeStream, streamFromIterable, concatArrayBuffer } from './stream-utils.js';
...
import { toString, toArrayBuffer } from 'lws';
import { formData as parseMultipart } from './multipart.js';
...
export class Body {
   ...
   * `ReadableStream` (via `readableStreamController()`, lib/lws/stream-
  ...
  static protocol(callback) {
    ...
    return {
      onHttpBody: (wsi, buf) => {
        ...
        if(!sink) {
          const [stream, s] = readableStreamController();
   ...
   * multipart/form-data, drains `this._multipart` (a `MultipartParser`,
   * set by `HttpProtocol` - lib/lws/protocols.js - when the request's
   * Content-Type is multipart) incrementally instead of buffering the
   * whole body as text - see `formData()` in lib/lws/multipart.js.
   *
  ...
  async formData() {
    ...
    if(/^multipart\/form-data/i.test(ct)) {
      if(!this._multipart) throw new TypeError('Body.formData: no MultipartParser available for this request');

      this.bodyUsed = true;
      return parseMultipart(this._multipart);
    }
