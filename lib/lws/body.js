...
export class Body {
  ...
  async arrayBuffer() {
    ...
    if(isPrototypeOf(ArrayBuffer.prototype, body)) return body;
    ...
    if(typeIsObject(body) && typeof body.arrayBuffer == 'function') return await body.arrayBuffer();
  ...
  async text() {
    ...
    if(isPrototypeOf(ArrayBuffer.prototype, body)) {
      const { TextEncoder } = globalThis.TextEncoder ? globalThis : await import('textcode');
      return new TextEncoder().decode(body);
    }

    if(typeIsObject(body) && typeof body.text == 'function') return await body.text();
