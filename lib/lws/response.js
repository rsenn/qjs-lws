...
export class Response extends Body {
  constructor(body, options = {}) {
    ...
    if(this.status < 200 || this.status > 599) throw new RangeError(`The status provided (${this.status}) is outside the range [200, 599].`);
  ...
  /**
   * HTTP status code. Deprecated alias for `status` property.
   * Kept for backward compatibility with code that used the old
   * non-standard `statusCode` getter.
   */
  get statusCode() {
    return this.status;
  }
