...
export class Response extends Body {
  ...
  /**
   * HTTP status code. Read-only; use `.status(code)` to change it.
   *
   * `HttpClientProtocol#onEstablishedClientHttp` (lib/lws/protocols.js)
   * patches the real status onto a client Response *after* construction
   * by assigning the plain `status` property directly (it can't reach
   * `#status`, a true private field) - which, since `status` is normally
   * the inherited `status(code)` method until that first assignment,
   * shadows it with a plain number from then on. Prefer that live,
   * patched-in value when present; fall back to `#status` (kept in sync
   * by the `status(code)` method itself, e.g. for ServerResponse, which
   * always uses that method and never gets patched this way) otherwise -
   * see BUGS: response-ok-stale-after-establish for why `#status` alone
   * used to read back the construction-time default forever. */
  get statusCode() {
    return typeof this.status === 'number' ? this.status : this.#status;
  }
  ...
  /**
   * Set the status code. Chainable, Express-style - shared with
   * `ServerResponse` (./response.js below), which relies on `status()`
   * being a method rather than a plain property so `res.status(code)...`
   * chains work.
   *
   * @param  {number}   code
  ...
  clone() {
    const { statusCode: status, statusText, headers, url, redirected, _bodyInit } = this;
...
export class ServerResponse extends Response {
  ...
  /**
   * Append a Set-Cookie header. Same header-building as `Response.cookie()`
   * (buildSetCookie, above) - only adds the headers-already-sent guard.
   *
   * @param  {string} name
   * @param  {string} value
   * @param  {object} [opts]   { maxAge, domain, path, expires, httpOnly,
  ...
  cookie(name, value, opts = {}) {
    this.#assertOpen();
    return super.cookie(name, value, opts);
  }
  ...
  #flushHeaders() {
    ...
    /* lws_add_http_common_headers() (native, called below via the `len`
       argument) already writes its own Content-Length header from `len`
       when one is known - leaving "content-length" in `headers` too made
       wsi.respond() write it a *second* time via lws_add_http_header_by_name,
       producing two Content-Length lines on the wire (a client's Headers
       then reports them comma-joined, e.g. "17,  17" - confirmed directly,
       pre-existing and independent of the `undefined`-arg fix below). */
    if(contentLength != null) delete headers['content-length'];

    /* wsi.respond()'s native arg scanner (lwsjs_socket_respond,
       lws-socket.c) stops at the first argument it doesn't recognize -
       an explicit `undefined` here (used to mean "no content-length")
       matches none of its number/ArrayBuffer/string/object cases, so it
       broke out of the scan before ever reaching `headers`, silently
       dropping every custom header (including Transfer-Encoding) on any
       response with no declared content-length. Omitting the argument
       entirely instead of passing `undefined` lets the scanner's
       positional matching fall through to the headers object as
       argv[1], same as it already does when a length *is* known. */
    if(contentLength != null) this.#wsi.respond(this.statusCode, Number(contentLength), headers);
    else this.#wsi.respond(this.statusCode, headers);
  }
