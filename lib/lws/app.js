...
const decoder = (() => {
  ...
  try { return new TextDecoder('utf-8'); } catch { return null; }
})();

function decodeBuf(buf) {
  if(decoder) try { return decoder.decode(buf); } catch {}
  return toString(buf);
}
...
export class ServerRequest {
  ...
  readBody(limit = 1 << 20) {
    return this.#bodyPromise.then(() => {
      ...
      let offset = 0;
      for(const c of this.#bodyChunks) { out.set(new Uint8Array(c), offset); offset += c.byteLength; }
      return (this.rawBody = out.buffer);
    });
  }

  readText(limit) { return this.readBody(limit).then(decodeBuf); }
  readJson(limit) { return this.readText(limit).then(s => (s ? JSON.parse(s) : null)); }

  /* Wired up by App from onHttpBody / onHttpBodyCompletion. */
  _appendBody(buf) { if(buf && buf.byteLength) this.#bodyChunks.push(buf); }
  _closeBody()     { this.#bodyResolve(); }
  _failBody(err)   { this.#bodyReject(err); }
}
...
export class ServerResponse {
  ...
  constructor(wsi) { this.#wsi = wsi; }

  get wsi()         { return this.#wsi; }
  get statusCode()  { return this.#status; }
  get headersSent() { return this.#headersSent; }
  get sent()        { return this.#ended; }
  get headers()     { return this.#headers; }

  status(code)            { this.#status = code; return this; }
  set(name, value)        { this.#assertOpen(); this.#headers.set(name, value); return this; }
  append(name, value)     { this.#assertOpen(); this.#headers.append(name, value); return this; }
  setHeader(name, value)  { return this.set(name, value); }
  getHeader(name)         { return this.#headers.get(name); }
  removeHeader(name)      { this.#assertOpen(); this.#headers.delete(name); return this; }
  type(contentType)       { return this.set('content-type', contentType); }
  ...
  redirect(...args) {
    let status = 302;
    let url = args[0];
    if(args.length > 1) { status = args[0]; url = args[1]; }
  ...
  send(body) {
    ...
    if(!this.#headers.has('content-type'))
      this.type(typeof body === 'string' ? 'text/html; charset=utf-8' : 'application/octet-stream');
...
export class App {
  ...
  use(arg, ...rest) {
    let path = '/', handlers = rest;

    if(typeof arg === 'string') { path = arg; }
    else                        { handlers = [arg, ...rest]; }
      ...
      if(h instanceof App) {
        ...
        this.layers.push(new Layer({
          ...
          handler: (req, res, next) => {
            ...
            return h.dispatch(req, res).then(handled => {
              req.path = saved;
              if(!handled) next();
            }).catch(err => {
              req.path = saved;
  ...
  _route(method, path, handlers) {
    ...
    for(const h of handlers.flat())
      if(typeof h === 'function')
        this.layers.push(new Layer({ method, prefix: '/', regex, keys, handler: h }));
    return this;
  }
    ...
    const run = async (err) => {
      while(i < layers.length) {
        ...
        for(let k = 0; k < layer.keys.length; k++)
          req.params[layer.keys[k]] = decodeURIComponent(m[k + 1]);
        ...
        try {
          let advanced = false;
          const localNext = (e) => { advanced = true; return run(e); };
      ...
      if(err) {
        responded = true;
        if(!res.headersSent) res.status(500).type('text/plain').end(String(err.stack || err.message || err));
      }
    };
  ...
  listen(opts = {}) {
    ...
    return new LWSContext({
      ...
      mounts: [
        ...mounts,
        { mountpoint: '/', protocol: '__app', originProtocol: LWSMPRO_CALLBACK },
      ],
      protocols: [
        ...protocols,
        {
          ...
          onHttp: (wsi) => {
            ...
            this.dispatch(req, res).then(handled => {
              if(!handled && !res.headersSent)
                res.status(404).type('text/plain').end('Not Found');
            }).catch(err => {
              if(!res.headersSent) {
                try { res.status(500).type('text/plain').end(String(err.stack || err)); } catch {}
              }
            });
          },
...
for(const m of METHODS)
  App.prototype[m] = function(path, ...handlers) { return this._route(m.toUpperCase(), path, handlers); };
