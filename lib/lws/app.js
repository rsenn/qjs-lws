...
import { LWSContext, LWSMPRO_CALLBACK, LWS_WRITE_HTTP, LWS_WRITE_HTTP_FINAL, toString, toArrayBuffer } from 'lws.so';
...
export class ServerRequest extends Body {
  ...
  constructor(wsi) {
    ...
    /* Set by HttpProtocol (protocols.js) when this request's Content-Type
       is multipart/form-data - lets formData() (body.js) drain it via
       MultipartParser instead of buffering the whole body as text. */
    this._multipart = undefined;
  }
...
export class ServerResponse {
   ...
   */
    ...
    if(!this.#headersSent && !this.#headers.has('content-length')) {
      const len = chunk == null ? 0 : typeof chunk === 'string' ? toArrayBuffer(chunk).byteLength : chunk.byteLength;
