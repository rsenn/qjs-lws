...
export class MemoryStore {
  ...
  async destroy(sid) { this.#map.delete(sid); }
  ...
  sweep() {
    const now = Date.now();
    let n = 0;
    for(const [sid, entry] of this.#map) if(entry.expires <= now) { this.#map.delete(sid); n++; }
    return n;
  }
...
export function session(opts = {}) {
  ...
  return async function sessionMiddleware(req, res, next) {
    ...
    def('destroy', async () => {
      destroyed = true;
      try { await store.destroy(sid); } catch {}
      for(const k of Object.keys(data)) delete data[k];
      if(!res.headersSent) res.clearCookie(name, { path: cookieOpts.path, domain: cookieOpts.domain });
    });

    def('save', async () => { await store.set(sid, snapshotKeys(data), ttlMs); });

    def('touch', async () => { await store.touch(sid, ttlMs); });
    ...
    def('regenerate', async () => {
      ...
      try { await store.set(sid, preserved, ttlMs); } catch {}
    });
