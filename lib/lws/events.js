import { weakMapper } from './util.js';
...
const eventHandlers = weakMapper(() => ({}));
...
export function EventTargetProperties(properties = []) {
  ...
  for(let prop of properties) {
    Object.defineProperty(ctor.prototype, 'on' + prop, {
      get() {
        return eventHandlers(this)[prop];
      },
      set(value) {
        const handlers = eventHandlers(this);
        if(handlers[prop]) this.removeListener(handlers[prop]);

        handlers[prop] = value;
        if(value) this.addEventListener(prop, value);
      },
    });
  }
