export class EventTarget {
   ...
   * @param {Object}   [options] Options object: { once, signal }
   */
  addEventListener(type, listener, options = {}) {
    ...
    // If already aborted, don't add listener
    if(signal && signal.aborted) return;

    // Create wrapper if once or signal is specified
    ...
    if(once || signal) {
      ...
      // Track mapping from original listener to wrapper
      if(!this.#listenerMap.has(listener)) {
        this.#listenerMap.set(listener, []);
      }
      this.#listenerMap.get(listener).push({ type, wrapper });
    }

    (this.#listeners[type] ??= []).push(actualListener);

    // Handle signal - remove listener when aborted
    if(signal) {
  ...
  removeEventListener(type, listener) {
    ...
    // Check if we have a wrapper for this listener
    if(this.#listenerMap.has(listener)) {
      const wrappers = this.#listenerMap.get(listener);
      for(let i = wrappers.length - 1; i >= 0; i--) {
        const { type: wType, wrapper } = wrappers[i];
        if(wType === type) {
          removeAll(this.#listeners[type], wrapper);
          wrappers.splice(i, 1);
          if(wrappers.length === 0) this.#listenerMap.delete(listener);
          break;
        }
      }
    } else {
      // No wrapper, remove the listener directly
  ...
  dispatchEvent(event) {
    ...
    /* Also fire if this EventTarget has an `on${EVENT_TYPE}` property that's
       a function - checked unconditionally (not just when a listener was
       also registered for `type`), since this is the only dispatch path for
       an EventTarget subclass with no addEventListener() call for `type`. */
