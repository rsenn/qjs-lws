...
export function fetch(url, options = {}) {
  const { cache, credentials, headers, method, mode, signal, ...rest } = options;
  options = rest;

  let resp = new Response(null, {}),
    req = new Request(url, { cache, credentials, headers, method, mode, signal });
      ...
      protocols: [
        {
          ...
          onEstablishedClientHttp(wsi, status) {
            ...
            verbose('onEstablishedClientHttp', { status });
          },
