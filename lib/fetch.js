...
import { getCallbackName, LCCSCF_ALLOW_SELFSIGNED, LCCSCF_H2_PRIOR_KNOWLEDGE, LCCSCF_HTTP_MULTIPART_MIME, LCCSCF_PIPELINE, LWS_SERVER_OPTION_ALLOW_NON_SSL_ON_SSL_PORT, LWS_SERVER_OPTION_CREATE_VHOST_SSL_CTX, LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT, LWS_SERVER_OPTION_IGNORE_MISSING_CERT, LWS_SERVER_OPTION_PEER_CERT_NOT_REQUIRED, LWS_SERVER_OPTION_REDIRECT_HTTP_TO_HTTPS, } from 'lws.so';
...
// Set by onClientHttpRedirect, consumed by the onClientHttpDropProtocol
// that immediately follows it - distinguishes "lws is following a
// redirect onto a new wsi" (expected) from a genuine abnormal drop.
...
function httpProtocol() {
  return {
    ...
    onClientHttpRedirect(wsi, url, status) {
      ...
      wsi.resp.redirected = true;

      /* lws follows the redirect itself, on a brand-new wsi - the old one
         gets dropped (onClientHttpDropProtocol) right after this. Refill
         `pending` with this call's still-live state so the new wsi's own
         transferPending() picks it up instead of finding nothing and
         leaving req/resp/resolve/reject permanently unattached. */
      redirecting = true;
      pending = { ctx: wsi.ctx, req: wsi.req, resp: wsi.resp, resolve: wsi.resolve, reject: wsi.reject, controller: wsi.controller, pwsi: wsi.pwsi };
    },
    ...
    onClosedClientHttp(wsi) {
      ...
      /* onEstablishedClientHttp never fired (e.g. the connection died
         before any response was ever received) - reject rather than
         resolving with a misleadingly-default Response. */
      if(!wsi.established) wsi.reject?.(new ConnectionError('closed'));
      /* Response was already resolved with valid status/headers, but the
         body stream never got closed (onCompletedClientHttp didn't fire) -
         error it so a caller awaiting resp.body doesn't hang forever. */ else if(!wsi.completed) wsi.controller.readable.error(new ConnectionError('closed'));
    ...
    onClientHttpDropProtocol(wsi) {
      ...
      /* A drop right after onClientHttpRedirect is lws switching to a new
         wsi to follow the redirect, not a failure - pending was just
         refilled for that new wsi, so leave it alone. */
      if(redirecting) {
        redirecting = false;
        return;
      }

      /* Otherwise the connection dropped without onCompletedClientHttp/
       * onClosedClientHttp/onClientConnectionError ever firing (e.g. the
       * server rejected/reset the request) - settle the promise and error
       * the body stream so callers don't hang forever instead of seeing a
       * rejection. Harmless no-op if already settled. */
