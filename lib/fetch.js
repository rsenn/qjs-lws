...
import { getCallbackName, LCCSCF_ALLOW_EXPIRED, LCCSCF_ALLOW_INSECURE, LCCSCF_ALLOW_SELFSIGNED, LCCSCF_H2_PRIOR_KNOWLEDGE, LCCSCF_HTTP_MULTIPART_MIME, LCCSCF_SKIP_SERVER_CERT_HOSTNAME_CHECK, LCCSCF_USE_SSL, LWS_SERVER_OPTION_ALLOW_NON_SSL_ON_SSL_PORT, LWS_SERVER_OPTION_CREATE_VHOST_SSL_CTX, LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT, LWS_SERVER_OPTION_H2_PRIOR_KNOWLEDGE, LWS_SERVER_OPTION_IGNORE_MISSING_CERT, LWS_SERVER_OPTION_PEER_CERT_NOT_REQUIRED, LWS_SERVER_OPTION_REDIRECT_HTTP_TO_HTTPS, LWS_WRITE_HTTP, LWS_WRITE_HTTP_FINAL, toString, write, } from 'lws';
      ...
      protocols: [
        {
          ...
          onClientAppendHandshakeHeader(wsi, data, len) {
            log('onClientAppendHandshakeHeader(1)', wsi, console.config({ maxArrayLength: 20 }), { data, len });

            for(let [name, value] of req.headers) {
              log('onClientAppendHandshakeHeader(2)', { name, value });
              wsi.addHeader(name, value, data, len);
            }

            log('onClientAppendHandshakeHeader(3)', wsi.method, { data: toString(data.slice(0, len[0])), len });
          ...
          onClientHttpWriteable(wsi, ...args) {
            log('onClientHttpWriteable', { wsi, args });

            /*const { bodyPending, method } = wsi;

            log('onClientHttpWriteable', wsi, { bodyPending, method, args });
