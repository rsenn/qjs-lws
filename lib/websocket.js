...
export class WebSocket extends EventTarget {
  ...
  static create(conn, makeContext = createContext) {
    const ws = new WebSocket();

    WebSocket.#create(ws, conn, makeContext);
  ...
  static #create(ws, conn, makeContext = createContext) {
    ...
    protocols: [
      {
        ...
        onClientClosed: (wsi, code = act(wsi).code, reason = act(wsi).reason) => (
          act(wsi).state(CLOSED).event('close', { code, reason }),
          0
        ),
