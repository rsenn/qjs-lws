...
import { ws } from './lws/protocols.js';
...
export class WebSocketStream {
  ...
  static protocol(name, callback) {
    return {
      name,
      ...ws({
        open: wsi => callback(new WebSocketStream(wsi, { ctor: ServerWebSocketAdapter })),
        message: (wsi, data, size) => {
          const adapter = wsiAdapters(wsi);

          adapter?.dispatchEvent({ type: 'message', target: adapter, data, size });
        },
        close: (wsi, code, reason) => {
          ...
          if(adapter) {
            ...
            adapter.dispatchEvent({ type: 'close', target: adapter, code, reason });
          }
        },
      }),
    };
...
/* wsi -> ServerWebSocketAdapter, so the ws() adapter's message/close
   handlers (see WebSocketStream.protocol() below) can route events to the
   right adapter instance without threading it through their own callback
   signatures. */
