...
import { client as wsClient, ws as wsServer, stream, reassembleFragments } from './lws/protocols.js';
...
export class WebSocketStream {
  ...
  static #ref = new ContextRefCounter(() => {
    WebSocketStream.#ctx?.destroy();
    WebSocketStream.#ctx = undefined;
    WebSocketStream.#adapter = undefined;
  });
  ...
  constructor(url, options = {}) {
    ...
    if(signal) signal.onabort = () => this.close();

    this.#url = String(url);

    const adapter = (WebSocketStream.#adapter ??= (() => {
      const a = stream({ extra: wsExtra, closeInfo: wsCloseInfo });
      a.message = reassembleFragments(a.message);
      return a;
    })());
    ...
    WebSocketStream.#ref.track(this, this.#opened, this.#closed);
  }
  ...
  static protocol(name, callback) {
    ...
    return {
      ...
      ...wsServer({
        ...
        message: reassembleFragments(adapter.message),
        close: adapter.close,
      }),
    };
  }
