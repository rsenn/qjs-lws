...
/** Flush a `Response` onto a `ServerResponse` (lib/lws/app.js), streaming its body if any. */
async function flush(resp, response) {
  resp.status(response.status);
  response.headers.forEach((value, name) => resp.append(name, value));

  if(response.body) for await (const chunk of response.body) resp.write(chunk);
...
export function serve(...args) {
  ...
  const { port = 0, hostname, host = hostname, tls, websocket = '/ws', raw = false, mounts, protocols = [], ...rest } = opts;
  ...
  const allProtocols = [{ name: 'http', ...http(handleRequest) }, ...protocols];
