...
import { isPrototypeOf, typeIsObject } from './lws/util.js';
...
/** `Request` -> client IP, backing `Server#requestIP()`. The `Request`
    handed to user code (toRequest()/requestFromWsi() below) doesn't carry
    its originating `wsi`, so the IP is captured here at construction time
    instead. */
const requestPeers = new WeakMap();
...
function toRequest(req) {
  ...
  requestPeers.set(request, req.wsi.peer?.host ?? null);
...
function requestFromWsi(wsi) {
  ...
  const request = new Request(url, { method: wsi.method || 'GET', headers });

  requestPeers.set(request, wsi.peer?.host ?? null);
...
function compileRoutes(routes) {
  ...
  for(const pattern in routes) {
    ...
    else if(entry && typeIsObject(entry)) {
...
class Server {
  ...
  constructor(ctx, port, hostname, publish, development = false, subscriberCount) {
    this.#ctx = ctx;
    this.port = port;
  ...
  stop() {
    // Bun's server.stop() returns a Promise that resolves when the server
    // has fully shut down. Our ctx.destroy() is synchronous (native call),
    // but we defer to a microtask to match Bun's async API and avoid any
    // potential issues with destroying while lws is still unwinding callbacks.
  ...
  subscriberCount(topic) {
    return this.#subscriberCount(topic);
  }

  /** Bun's server.requestIP(request) - returns the client IP address for a request.
      Returns null if unavailable. */
  requestIP(request) {
    return requestPeers.get(request) ?? null;
  }

  /** Bun's server.timeout(seconds) - sets the server-wide idle timeout.
      Applied via wsi.setTimeout() (native lws_set_timeout()) to each
      connection as it's accepted/re-armed on activity - see _applyTimeout()
      below. `0` disables it. */
  timeout(seconds) {
  ...
  /** Re-arms the idle timeout set by timeout() (if any) on `wsi` - called
      on each new connection/request/message so the timeout means "idle
      for N seconds", not "N seconds since connect". No-op if timeout()
      was never called. */
  _applyTimeout(wsi) {
    if(this.#timeoutSeconds) wsi.setTimeout(this.#timeoutSeconds);
  }
}
...
export function serve(...args) {
  ...
  for(const arg of args) {
    ...
    else if(arg && typeIsObject(arg)) Object.assign(opts, arg);
  }

  fetchHandler ??= opts.fetch;

  const { port = 0, hostname, host = hostname, tls, websocket = '/ws', raw = false, mounts, protocols = [], headers, html, access, upgrade, auth, routes, development = false, ...rest } = opts;
  ...
  const wsOpen = websocket && typeIsObject(websocket) ? websocket.open : undefined;
  const wsMessage = websocket && typeIsObject(websocket) ? websocket.message : undefined;
  const wsClose = websocket && typeIsObject(websocket) ? websocket.close : undefined;
  const wsDrain = websocket && typeIsObject(websocket) ? websocket.drain : undefined;
  const wsPing = websocket && typeIsObject(websocket) ? websocket.ping : undefined;
  const wsPong = websocket && typeIsObject(websocket) ? websocket.pong : undefined;
  const wsIdleTimeout = websocket && typeIsObject(websocket) ? websocket.idleTimeout : undefined;

  /* NOT YET IMPLEMENTED: accepted here (so passing them isn't an error) but
     never read again below - no compression, no payload-size cap, no
     backpressure-limit enforcement, no ping suppression, no publish-to-self.
     See BUGS: serve-websocket-options-silently-ignored. */
  const wsPerMessageDeflate = websocket && typeIsObject(websocket) ? websocket.perMessageDeflate : undefined;
  const wsMaxPayloadLength = websocket && typeIsObject(websocket) ? websocket.maxPayloadLength : undefined;
  const wsBackpressureLimit = websocket && typeIsObject(websocket) ? websocket.backpressureLimit : undefined;
  const wsCloseOnBackpressureLimit = websocket && typeIsObject(websocket) ? websocket.closeOnBackpressureLimit : undefined;
  const wsSendPings = websocket && typeIsObject(websocket) ? websocket.sendPings : undefined;
  const wsPublishToSelf = websocket && typeIsObject(websocket) ? websocket.publishToSelf : undefined;

  const wsBunStyle = !!(wsOpen || wsMessage || wsClose || wsDrain || wsPing || wsPong);
  ...
  if(wsPath !== false) {
    wsDescriptor = wsBunStyle
      ? WebSocket.protocol('ws', ws => {
          ...
          // websocket.idleTimeout (handler-specific) is applied after and so
          // takes precedence over server.timeout() (server-wide) when both
          // are set - wsi.setTimeout() just overwrites the pending timer.
          const applyIdleTimeout = () => {
            server._applyTimeout(wsi);
            if(wsIdleTimeout) wsi.setTimeout(wsIdleTimeout);
          };

          applyIdleTimeout();
          ws.addEventListener('message', applyIdleTimeout);

          wsOpen?.(ws);
          if(wsMessage) ws.onmessage = e => wsMessage(ws, e.data);
          if(wsClose) ws.onclose = e => wsClose(ws, e.code, e.reason);
          if(wsPing) ws.addEventListener('ping', e => wsPing(ws, e.data));
          if(wsPong) ws.addEventListener('pong', e => wsPong(ws, e.data));
          if(wsDrain) ws.addEventListener('drain', () => wsDrain(ws));
        })
  ...
  server = new Server(ctx, actualPort, host, wsDescriptor?.publish, development, wsDescriptor?.subscriberCount);
