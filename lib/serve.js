...
import { LWSMPRO_CALLBACK, LWSMPRO_NO_MOUNT, LWS_SERVER_OPTION_FALLBACK_TO_APPLY_LISTEN_ACCEPT_CONFIG, LWS_SERVER_OPTION_ADOPT_APPLY_LISTEN_ACCEPT_CONFIG, toArrayBuffer } from 'lws.so';
...
class Server {
  ...
  constructor(ctx, port, hostname, publish) {
    this.#ctx = ctx;
    this.port = port;
...
export function serve(...args) {
  ...
  server = new Server(ctx, port, host, wsDescriptor?.publish);
  ...
  return Object.assign(sink, { context: ctx, stop: () => ctx.destroy(), port, hostname: host, publish: wsDescriptor?.publish ?? (() => 0), upgrade: () => false });
}
