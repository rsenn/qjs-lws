...
function toRequest(req) {
  ...
  // req.stream() (lib/lws/app.js) enqueues each chunk as it's read off the
  // socket - unlike req.readBody(), it doesn't wait for (or buffer) the
  // whole body before the first byte is visible here.
  const body = NO_BODY_METHODS.has(req.method) ? undefined : req.stream();
...
export function serve(...args) {
  ...
  const rawEntry = rawProtocol !== false ? TCPSocket.protocol(rawProtocol, socket => (fetchHandler ? fetchHandler(socket) : sink.push(socket))) : null;
  ...
  const ctx = createContext({
    ...
    ...(tls ? { tls } : {}),
    ...(raw !== false
      ? { listenAcceptRole: 'raw-skt', listenAcceptProtocol: rawProtocol, options: (rest.options ?? 0) | (rawAlways ? LWS_SERVER_OPTION_ADOPT_APPLY_LISTEN_ACCEPT_CONFIG : LWS_SERVER_OPTION_FALLBACK_TO_APPLY_LISTEN_ACCEPT_CONFIG) }
      : {}),
    ...rest,
    mounts: allMounts,
